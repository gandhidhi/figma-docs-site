...
- [ ] 2. コンテンツアダプター層の実装
  - [ ] 2.1 `src/adapters/types.ts` を作成して `DocPage` インターフェースと `IContentAdapter` インターフェースを定義する
    - `getAllPages()`, `getPageBySlug()`, `getPagesByCategory()` メソッドを定義する
    - _要件: 4.1, 4.2_
  - [ ] 2.2 `src/adapters/local-mdx.ts` を作成して `LocalMDXAdapter` クラスを実装する
  ...
  - [ ] 2.5 `src/adapters/headless-cms.ts` を作成してフェーズ2用の `HeadlessCMSAdapter` スタブを実装する
    - `IContentAdapter` を実装し、全メソッドは `throw new Error('Not implemented')` とする
    - _要件: （フェーズ2対応）_

- [ ] 3. Content Collectionsスキーマ定義
  - [ ] 3.1 `src/content/config.ts` を作成して `docsSchema` を使用したコレクション定義を実装する
...
- [ ] 4. チェックポイント — アダプター層の動作確認
  - 全テストが通ることを確認する。疑問点があればユーザーに確認する。

- [ ] 5. サイドバーナビゲーション設定
  - [ ] 5.1 `astro.config.mjs` のサイドバー設定に4つのトップレベルカテゴリを定義する
...
- [ ] 6. 「はじめに」カテゴリのMDXコンテンツ作成
  - [ ] 6.1 `src/content/docs/getting-started/what-is-figma.mdx` を作成する
    - frontmatter: `title: Figmaとは`, `description` を設定する
    - Figmaの概要とユースケースを記述する
    - _要件: 5.1_
  - [ ] 6.2 `src/content/docs/getting-started/installation.mdx` を作成する
    - frontmatter: `title: インストール方法`, `description` を設定する
    - Starlightの `Steps` コンポーネントと `Tabs` コンポーネントを使用してWindows/macOS/ブラウザの手順を記述する
    - _要件: 5.2, 5.4_
  - [ ] 6.3 `src/content/docs/getting-started/account-setup.mdx` を作成する
    - frontmatter: `title: アカウント作成とログイン`, `description` を設定する
    - アカウント登録とログイン手順を記述する
    - _要件: 5.3_

- [ ] 7. 「基本操作」カテゴリのMDXコンテンツ作成
  - [ ] 7.1 `src/content/docs/basics/ui-overview.mdx` を作成する（`title: 画面構成と各部名称`）
    - _要件: 6.1_
  - [ ] 7.2 `src/content/docs/basics/file-management.mdx` を作成する（`title: ファイルとプロジェクトの管理`）
    - _要件: 6.2_
  - [ ] 7.3 `src/content/docs/basics/frames-and-layers.mdx` を作成する（`title: フレームとレイヤー`）
    - _要件: 6.3_
  - [ ] 7.4 `src/content/docs/basics/shapes-text-images.mdx` を作成する（`title: 図形・テキスト・画像の配置`）
    - _要件: 6.4_
  - [ ] 7.5 `src/content/docs/basics/components-basics.mdx` を作成する（`title: コンポーネントの基本`）
    - _要件: 6.5_

- [ ] 8. 「応用操作」カテゴリのMDXコンテンツ作成
  - [ ] 8.1 `src/content/docs/advanced/auto-layout.mdx` を作成する（`title: オートレイアウト`）
    - _要件: 7.1_
  - [ ] 8.2 `src/content/docs/advanced/prototypes.mdx` を作成する（`title: プロトタイプとインタラクション`）
    - _要件: 7.2_
  - [ ] 8.3 `src/content/docs/advanced/team-sharing.mdx` を作成する（`title: チーム共有とコメント`）
    - _要件: 7.3_
  - [ ] 8.4 `src/content/docs/advanced/plugins.mdx` を作成する（`title: プラグインの活用`）
    - _要件: 7.4_
  - [ ] 8.5 `src/content/docs/advanced/design-tokens.mdx` を作成する（`title: デザイントークンとスタイル管理`）
    - _要件: 7.5_

- [ ] 9. 「リファレンス」カテゴリのMDXコンテンツ作成
  - [ ] 9.1 `src/content/docs/reference/shortcuts.mdx` を作成する（`title: ショートカットキー一覧`）
    - 操作名・Windows・macOSの3列テーブル形式でショートカットを記述する
    - _要件: 8.1, 8.3_
  - [ ] 9.2 `src/content/docs/reference/glossary.mdx` を作成する（`title: 用語集`）
    - Figma固有の主要用語を定義する
    - _要件: 8.2_

- [ ] 10. チェックポイント — コンテンツ網羅性の確認
  - 全テストが通ることを確認する。疑問点があればユーザーに確認する。

- [ ] 11. ファイル構造テストの実装
  - [ ] 11.1 `tests/unit/content-structure.test.ts` を作成して全16ページのMDXファイル存在確認テストを実装する
